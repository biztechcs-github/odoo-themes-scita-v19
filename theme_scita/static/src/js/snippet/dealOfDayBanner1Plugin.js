...
// Shared Modal Logic
// -------------------------------------------------
async function openDealOfDayBanner1Modal(snippetEl) {
...
// Snippet Drop Plugin
// -------------------------------------------------
class DealOfDayBanner1SnippetPlugin extends Plugin {
    static id = "dealOfDayBanner1Snippet";
    static dependencies = [];

    resources = {
        on_snippet_dropped_handlers: this.onSnippetDropped.bind(this),
        so_content_addition_selector: [".retial_deal_of_day_banner_snippet_1"],
    };

    async onSnippetDropped({ snippetEl }) {
        if (!snippetEl.classList.contains("retial_deal_of_day_banner_snippet_1")) {
            return;
        }

        // open modal after drop
        await openDealOfDayBanner1Modal(snippetEl);
    }
}
registry.category("website-plugins").add(
    DealOfDayBanner1SnippetPlugin.id,
    DealOfDayBanner1SnippetPlugin
);
