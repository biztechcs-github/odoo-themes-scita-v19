...
4. **Initialize verification report structure**
   ...
   - **CI, Test & Runtime**: Track unfinished-code markers, test file coverage, and proof the app was actually booted and smoke-tested (not just built)
...
8. **CI, Test & Runtime Gate**
   ...
   **Runtime validation gate (CRITICAL):**
   Determine whether this change touches a runnable surface — a service, an API, or a UI (anything with a start/run script or a served route). If it does, confirm there is evidence it was actually **booted and smoke-tested**, not merely built:
   - Acceptable evidence is a completed DoD task (or a runtime-evidence note in the change folder) showing: the app booted; a liveness probe (health route / GraphQL introspection / homepage) returned 2xx; any introduced file/asset URL round-tripped to 2xx with the right content-type; and each changed UI route loaded with no uncaught console errors and no failed (4xx/5xx) requests.
   - If the surface is runnable but there is **no such evidence** → add CRITICAL: "Runtime validation missing — the app was not booted and smoke-tested; a green build and passing unit tests do not prove it runs or that served assets resolve."
   - A DoD task that defers the run to a human (e.g. "manual end-to-end run — not performed in this environment") left unchecked is NOT acceptable evidence → treat as CRITICAL.
   - Only a change with no runnable surface (pure docs/config with no runtime effect) may note "No runnable surface — runtime validation N/A."

   **Do not skip this step.** A `TODO` or `mock` in production code is a CRITICAL gate failure regardless of all other checks passing — and so is a runnable change with no evidence it was booted.
...
11. **Generate Verification Report**
   ...
   **Summary Scorecard**:
   ...
   | CI, Test & Runtime | N files scanned, K stubs, runtime PASS/FAIL |
